/** Ensure report filter masters are warm (silent). */
...
const REPORT_RESOURCES = ['locations', 'entities', 'departments', 'units', 'categories'] as const

export function ensureReportFiltersBundle(): Promise<void> {
  return Promise.allSettled(
    REPORT_RESOURCES.map((r) => ensureCatalog(r, () => listAllSilent(r))),
  ).then(() => undefined)
}
